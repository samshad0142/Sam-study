...
DB = Path(os.getenv("SAMSTUDY_DB_PATH", str(DATA_DIR / "samstudy.db")))
...
def init_db():
    with db() as con:
        ...
        CREATE TABLE IF NOT EXISTS profiles (
          uid TEXT PRIMARY KEY, email TEXT, name TEXT DEFAULT '', purpose TEXT DEFAULT '', course TEXT DEFAULT '',
        ...
        """)
        try:
            con.execute("ALTER TABLE profiles ADD COLUMN purpose TEXT DEFAULT ''")
...
def render_page(template_name, **context):
    return render_template(template_name, firebase_config=json.dumps({
    ...
    }), admin_emails=sorted(ADMIN_EMAILS), youtube=os.getenv("YOUTUBE_URL", "https://youtube.com/@sam_malik77"), **context)
...
@app.get("/batches")
def batches_page():
    return render_page("pages/batches.html")


@app.get("/notes")
def notes_page():
    return render_page("pages/notes.html")


@app.get("/quiz")
def quiz_page():
    return render_page("pages/quiz.html")


@app.get("/tests")
def tests_page():
    return render_page("pages/tests.html")


@app.get("/three")
def three_page():
    return render_page("pages/three.html")


@app.get("/doubt")
def doubt_page():
    return render_page("pages/doubt.html")
...
def get_profile():
    ...
    return jsonify(profile={"uid":uid,"email":request.user.get("email",""),"name":request.user.get("name",""),
                            "purpose":"","course":"","exam":"","photo":request.user.get("picture","")},
...
def save_profile():
    ...
    course=(request.form.get("course") or "").strip() if purpose == "course" else ""
    exam=(request.form.get("exam") or "").strip() if purpose == "government" else ""
    photo=""
    ...
    with db() as con:
        ...
        con.execute("""INSERT INTO profiles(uid,email,name,purpose,course,exam,photo) VALUES(?,?,?,?,?,?,?)
                       ON CONFLICT(uid) DO UPDATE SET email=excluded.email,name=excluded.name,purpose=excluded.purpose,
                       course=excluded.course,exam=excluded.exam,photo=excluded.photo,
                       updated_at=CURRENT_TIMESTAMP""",
                    (uid,email,name,purpose,course,exam,photo))
        con.commit()
    return jsonify(ok=True, profile={"uid":uid,"email":email,"name":name,"purpose":purpose,"course":course,"exam":exam,"photo":photo})
...
def gemini_parts(parts):
    ...
    # Render may still contain an older GEMINI_MODEL value. Normalize old
    # 2.5 Flash values to the current stable 3.6 Flash model.
